...
$comptes = $statement->fetch(PDO::FETCH_OBJ);
##########################################

if (isset($_POST['modc'])) {

if(!empty(trim($_POST['idm']))){

$requete=$bdd->prepare('UPDATE `compte` SET `idcompte`=?,`nom`=?,`type`=?,`mail`=?, `service`=?, `motdepasse`=? WHERE `idcompte`=?');
$requete->execute(array($_POST['idm'],$_POST['nomm'],$_POST['rolem'],$_POST['mailm'],$_POST['serm'],$_POST['mdpm'],$id));
...
<div class="col-md-4">    
 ...
 <div class="col-md-4">                             
          ...
          <h4 style="font-size:20px;color:white;text-align: center;"><i><b>Modifier Profile</b></i></h4>
        </div>
 ...
 <div class="modal-body" style="padding:40px 50px;">
          <form role="form" action="" method="post" >  
           ...
           <input type="text" class="form-control" placeholder="id" name="idm" required value="<?= $comptes->idcompte; ?>" required >
          </div>
           <div class="form-group">
           <label for="usrname">NOM</label>
           <input type="text" class="form-control" placeholder="nom" name="nomm" required value="<?= $comptes->nom; ?>">
          </div>

            <div class="form-group">
           <label for="usrname">SERVICE</label>
           <select  class="form-control" name="serm" required value="<?= $comptes->service; ?>"> 
...
if (strcmp( $_SESSION['type'], "Admin") == 0) {
        echo'
          <div class="form-group">
           <label for="usrname">ROLE</label>
           <select  class="form-control"  name="rolem" required value="<?= $comptes->type; ?>" > 
                  <option>Admin</option>
                  <option>User</option>
          ...
          <div class="form-group">
           ...
           <input type="email" class="form-control" placeholder="email" name="mailm" value="<?= $comptes->mail; ?>">
          </div>

          <div class="form-group">
           <label for="usrname">MOT DE PASSE</label>
           <input type="text" class="form-control" placeholder="mdp" name="mdpm"  value="<?= $comptes->motdepasse;?>" required>
          </div>
