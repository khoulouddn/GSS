...
$id = $_GET['grp'];
$sql = 'SELECT * FROM `groupe-partage` WHERE `idgrppartage`=:id';
$statement = $bdd->prepare($sql);
$statement->execute([':id' => $id]);
$groupe = $statement->fetch(PDO::FETCH_OBJ);
##########################################

if (isset($_POST['modifg'])) {

 if( !empty(trim($_POST['nomgp'])) )
 {


$requete=$bdd->prepare('UPDATE `groupe-partage` SET `nom`=? ,`autresdroits`=?,`droits`=? WHERE
 `idgrppartage`=?');
$requete->execute(array($_POST['nomgp'],$_POST['autregp'],$_POST['drgp'],$id));
$requete1=$requete->fetch();
echo '<script>alert("la modification est bien effectuée");</script>';
}

$query ="SELECT * FROM `groupe-partage`  WHERE `idgrppartage`='".$_GET['grp']."'";
$stat = $bdd->query($query);
$tab = $stat->fetchAll ();
foreach($tab as $row)
{
$v=$row["fichier partage_idfichierpartage"]; 
}

header("Location:groupe-par.php?fif=$v");
}
?> 
...
<div class="col-md-4">    
 ...
 <div class="col-md-4">                             
          ...
          <h4 style="font-size:20px;color:white;text-align: center;"><i><b>modifier Groupe</b></i></h4>
        </div>
 ...
 <div class="modal-body" style="padding:40px 50px;">
        ...
        <div class="form-group">
           ...
           <input type="text" class="form-control" placeholder="nom" name="nomgp" value="<?= $groupe->nom;?>">
          </div>

           <div class="form-group">
           <label for="usrname">Droits</label>
           <select  class="form-control" name="drgp"> 
                 <option> </option>
          ...
          <div class="form-group">
           ...
           <input type="text" class="form-control" placeholder="autre" name="autregp" value="<?= $groupe->autresdroits; ?>">
          </div></br>


                

        <button type="submit" class="btn btn-success" name="modifg">Modifier</button>
        <br><br>
