...
header("Location:grp-app.php?adminaa=$id");
}
?> 
...
<body>
  ...
  <?php
if (strcmp( $_SESSION['type'], "Admin") == 0) {
        echo'<div  style="margin-left:1420px;">
...
$bdd= new PDO("mysql:host=localhost;dbname=gss;charset=utf8","root","");
$conn = mysqli_connect("localhost", "root", "", "gss"); 
$query = "SELECT * FROM `groupe-admin`  WHERE `serveur_idserveur` = '".$_GET['adminaa']."'" ;
$stat = $bdd->query($query);
$tab = $stat->fetchAll ();
foreach($tab as $row)
{
echo "<tr>";
if (strcmp( $_SESSION['type'], "Admin") == 0) {echo  '<td> <input type="checkbox" name="chc[]" value="'.$row["idadmin"];
 echo'"> </td>';}
...
if (strcmp( $_SESSION['type'], "Admin") == 0) { echo '<td> <a href="modifier-grp-app.php?moda='.$row['idadmin'].'"  data-toggle="modal1" data-target="#my"> 
