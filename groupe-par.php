...
if (isset($_POST['envoig']) ){
 $id = $_GET['fif'];
 if(!empty(trim($_POST['nomg'])))
 {
$recherche="SELECT `idgrppartage ` FROM `groupe-partage` WHERE `nom`='".$_POST['nomg']."'"; 
...
if (mysqli_num_rows($rs)<=0) { 
 $requete=$bdd->prepare('INSERT INTO `groupe-partage`( `nom`,`autresdroits`,`fichier partage_idfichierpartage` , `droits`) VALUES (?,?,?,?)');
 $requete->execute(array($_POST['nomg'],$_POST['autreg'],$id,$_POST['drg']));
...
header("Location:groupe-par.php?fif=$id");
}
?>
...
$query ="SELECT * FROM `groupe-partage` JOIN `fichier partage` WHERE `fichier partage_idfichierpartage` LIKE `idfichierpartage` AND       
`idfichierpartage`='".$_GET['fif']."'";
...
$recherche1="SELECT * FROM `groupe-partage` JOIN `fichier partage` WHERE `fichier partage_idfichierpartage` LIKE `idfichierpartage` AND       `idfichierpartage`='".$_GET['fif']."'"; 
...
<div style="text-align: center; color: #2b3856;"> 
  <b style="font-size:40px;">GROUPES PARTAGE</b></div>
...
$bdd= new PDO("mysql:host=localhost;dbname=gss;charset=utf8","root","");
$conn = mysqli_connect("localhost", "root", "", "gss"); 
$query = "SELECT * FROM `groupe-partage`  WHERE `fichier partage_idfichierpartage`='".$_GET['fif']."'" ;
$stat = $bdd->query($query);
$tab = $stat->fetchAll ();
foreach($tab as $row)
{
...
echo "<tr>";
if (strcmp( $_SESSION['type'], "Admin") == 0) { echo  '<td> <input type="checkbox" name="chc[]" value="'.$row["idgrppartage"];
 echo'"> </td>';}
echo '<td>' .$row["nom"]; echo '</td>';
echo '<td>'.$row['droits'];echo'</td>';
echo '<td>' .$row["autresdroits"]; echo '</td>';
echo' <td> <a href="membre.php?mem='.$row["idgrppartage"].'"class ="btn btn-default" ><i>membre</i></td>';
  echo '<td> <a href="modifier-groupepar.php?grp='.$row['idgrppartage'].'"  data-toggle="modal1" data-target="#my">
 ...
 $('#btn_delete').click(function(){
  ...
  else
  {
   return false;
                ...
                <h4 style="font-size:20px;color:white;text-align: center;"><span class="fa fa-user-plus"></span><i><b>Ajouter Groupe</b></i></h4></div>
          ...
          <div class="form-group">
           ...
           <input type="text" class="form-control" placeholder="nom" name="nomg" required>
          </div>

           <div class="form-group">
           <label for="usrname">Droits</label>
           <select  class="form-control" name="drg"> 
                  <option> </option>
                  <option>Lecteur</option>
                  <option>Ecriture</option>
                  <option>AccèsTotal</option>
            </select>
          </div></br>

          <div class="form-group">
           <label for="usrname">Autre Droits</label>
           <input type="text" class="form-control" placeholder="autre" name="autreg">
          </div></br>


                

        <button type="submit" class="btn btn-success" name="envoig"> Ajouter</button>
        <br><br>
