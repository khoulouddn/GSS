...
$id = $_GET['modsa'];
$sql = 'SELECT * FROM `serveur` JOIN `caracteristiques`  WHERE `idserveur`=:id AND `serveur_idserveur`=:id ';
...
if(!empty(trim($_POST['id']))){ 
	if (strcmp( $_POST['etat'], "connecté") == 0) {$h="sac.png";}else{$h="san.png";}

$requete8=$bdd->prepare('UPDATE `serveur` SET `idserveur`=?,`nom`=?,`IP`=?, `DNS`=? ,`etat`=? ,`localisation`=? ,`passerelle`=? ,
  `dernieremodification`=? ,`dureegarantie`=? ,`avatar`=?,`serveur dapplications_idserveur`=?,
  `serveur de partage_idserveur`=?  WHERE `idserveur`=?');
$requete8->execute(array($_POST['id'],$_POST['nom'],$_POST['ip'],$_POST['dns'],$_POST['etat'],$_POST['loc'],$_POST['pas'],
  $_POST['der'],$_POST['gar'],$h,$_POST['id'],'',$id));
$requete9=$requete8->fetch();

$requete=$bdd->prepare('UPDATE `caracteristiques` SET `marque`=?,`type`=?,`adresseMAC`=?, `RAM`=? ,`CPU`=? ,`espacedisque`=? ,
  `nbrcartesreseaux`=? ,`OS`=? ,`typeos`=? ,`versionantivirus`=? ,`serveur_idserveur`=?  WHERE `serveur_idserveur`=?');
$requete->execute(array($_POST['mar'],$_POST['type'],$_POST['mac'],$_POST['ram'],$_POST['cpu'],$_POST['dd'],$_POST['res'],
  $_POST['os'],$_POST['mod'],$_POST['ver'],$_POST['id'],$id));
$requete1=$requete->fetch();

 $requete5=$bdd->prepare('UPDATE `serveur relier` SET `idserveur`=?,`nom`=?,`IP`=?, `DNS`=? ,`etat`=? ,`localisation`=? ,`passerelle`=? ,`dernieremodification`=? ,`dureegarantie`=?,`marque`=?,`type`=?,`adresseMAC`=?, `RAM`=? ,`CPU`=? ,`espacedisque`=? ,`nbrcartesreseaux`=? ,`OS`=? ,`typeos`=? ,`versionantivirus`=? WHERE `idserveur`=?');
 $requete5->execute(array($_POST['id'],$_POST['nom'],$_POST['ip'],$_POST['dns'],$_POST['etat'],$_POST['loc'],$_POST['pas'],
  $_POST['der'],$_POST['gar'],$_POST['mar'],$_POST['type'],$_POST['mac'],$_POST['ram'],$_POST['cpu'],$_POST['dd'],$_POST['res'],
  $_POST['os'],$_POST['mod'],$_POST['ver'],$id));
 $requete2=$requete5->fetch();
echo '<script>alert("la modification est bien effectuée");</script>';

}
header('Location:liste-serveurs-app.php');
}
?> 
<!DOCTYPE html>
...
<div class="col-md-4">    
 ...
 <div class="col-md-4">                             
          ...
          <h4 style="font-size:20px;color:white;text-align: center;"><i><b>modifier un Serveur</b></i></h4>
        </div>
 ...
 <div class="modal-body" style="padding:40px 50px;">
          ...
          <div class="form-group">
           ...
           <input type="number" class="form-control" placeholder="carte reseau" name="res" value="<?= $srv->nbrcartesreseaux;?>">
            ...
            <div class="form-group">
           <label for="usrname" >Système D'expmoitation</label>
