...
$id = $_GET['modap'];
$sql = 'SELECT * FROM `applications` WHERE `idapp`=:id';
$statement = $bdd->prepare($sql);
$statement->execute([':id' => $id]);
$appli = $statement->fetch(PDO::FETCH_OBJ);
##########################################
...
$requete=$bdd->prepare('UPDATE `applications` SET `version`=?, `description`=? ,`nom`=? WHERE `idapp`=?');
$requete->execute(array($_POST['ver'],$_POST['des'],$_POST['noma'],$id));
$requete1=$requete->fetch();
echo '<script>alert("la modification est bien effectuée");</script>';
} 
$query ="SELECT * FROM `applications` WHERE `idapp`='".$_GET['modap']."'";
$stat = $bdd->query($query);
$tab = $stat->fetchAll ();
foreach($tab as $row)
{
$v=$row["serveur dapplications_idserveur"]; 
}

header("Location:liste-app.php?a=$v");
}
?> 
<!DOCTYPE html>
        ...
        <header class="site-header container-fluid">
            <div class="top-header">
               ...
               <div class="social-top" col-md-6 col-sm-6>
                    <ul>
                        <?php  
$query1 ="SELECT * FROM `applications` WHERE `idapp`='".$_GET['modap']."'";
$stat1 = $bdd->query($query1);
$tab1 = $stat1->fetchAll ();
foreach($tab1 as $row)
{
$n=$row["serveur dapplications_idserveur"]; 
}
   echo '<li style="margin-right:3px;"><a href="liste-app.php?a='.$n.'" class="fa fa-angle-left" title="Précédent"></a></li>';
                      session_start();
                      $r=session_id();
...
<div class="col-md-4">    
 ...
 <div class="col-md-4">                             
          ...
          <h4 style="font-size:20px;color:white;text-align: center;"><i><b>modifier L'application</b></i></h4>
        </div>
 ...
 <div class="modal-body" style="padding:40px 50px;">
        ...
        <div class="form-group">
           ...
           <input type="text" class="form-control" placeholder="nom" name="noma" value="<?= $appli->nom;?>" required>
          </div>

          <div class="form-group">
           <label for="usrname"> VERSION</label>
           <input type="text" class="form-control" placeholder="version" name="ver" value="<?= $appli->version;?>">
          </div>

          <div class="form-group">
           <label for="usrname">DESCRIPTION</label>
           <input type="text" class="form-control" placeholder="description" name="des" value="<?= $appli->description;?>">
          </div>

        <button type="submit" class="btn btn-success" name="modapp">Modifier</button>
        <br><br>
